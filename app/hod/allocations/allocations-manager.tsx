...
import { saveHodAllocation, deleteHodAllocation, type HodAllocState } from "./actions";
import { Button, Field, Select, Input, Alert, Badge } from "@/components/ui";
...
type Course = { id: number; code: string; title: string; level: number };
type Lecturer = { id: number; name: string; departmentName: string; isOwnDepartment: boolean };
type Venue = { id: number; name: string; capacity: number };

export type HodAllocRow = {
  id: number;
...
export function HodAllocationsManager({
  departmentName, rows, courses, lecturers, venues,
}: {
  departmentName: string;
  rows: HodAllocRow[]; courses: Course[]; lecturers: Lecturer[]; venues: Venue[];
}) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [editing, setEditing] = useState<HodAllocRow | null>(null);
  const [state, formAction] = useFormState<HodAllocState, FormData>(saveHodAllocation, {});
  ...
  const ownDept = lecturers.filter((l) => l.isOwnDepartment);
  const otherDept = lecturers.filter((l) => !l.isOwnDepartment);

  return (
    <>
      <PageHeader
        title="Department classes"
        subtitle={`${departmentName}. Replace a lecturer, move a slot, or add a class. Every change is conflict-checked.`}
      ...
      <Table>
        <THead>
          <TR className="hover:bg-transparent">
            <TH>Course</TH><TH>Lecturer</TH><TH>Day</TH><TH>Time</TH><TH>Venue</TH>
        ...
        <TBody>
          {rows.length === 0 ? (
            <EmptyRow colSpan={6}>
              No classes in {departmentName} yet. Add one, or run the generator.
            </EmptyRow>
          ) : (
            rows.map((r) => (
              <TR key={r.id}>
                ...
                <TD className="text-right">
                  <div className="flex items-center justify-end gap-3">
                    ...
                    <form
                      action={deleteHodAllocation}
                      onSubmit={(e) => {
                        if (!confirm("Remove this class?")) e.preventDefault();
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={editing ? "Edit class" : "Add class"}>
        <form action={formAction}>
          ...
          <Field label="Course" htmlFor="courseId" hint={`Only ${departmentName} courses.`}>
            <Select id="courseId" name="courseId" required value={courseId} onChange={(e) => setCourseId(e.target.value)}>
              <option value="" disabled>Select a course…</option>
              {courses.map((c) => (
                <option key={c.id} value={c.id}>{c.code} · {c.title} ({c.level} Level)</option>
              ))}
            </Select>
          </Field>
          ...
          <Field
            label="Lecturer"
            htmlFor="lecturerId"
            hint="Change this to replace the teacher. Both lecturers are notified."
          >
            <Select id="lecturerId" name="lecturerId" required value={lecturerId} onChange={(e) => setLecturerId(e.target.value)}>
              ...
              {ownDept.length > 0 && (
                <optgroup label={departmentName}>
                  {ownDept.map((l) => (<option key={l.id} value={l.id}>{l.name}</option>))}
                </optgroup>
              )}
              {otherDept.length > 0 && (
                <optgroup label="Other departments">
                  {otherDept.map((l) => (
                    <option key={l.id} value={l.id}>{l.name} ({l.departmentName})</option>
                  ))}
                </optgroup>
              )}
            </Select>
